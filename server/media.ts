...
export function mediaLibraryScript() {
  return `(() => {
    ...
    init: async ({ handleInsert } = {}) => {
      ...
      const show = async () => {
        ...
        uploadButton.addEventListener("click", async () => {
          ...
          if (files.some(file => file.size > MAX_FILE_BYTES)) return setStatus("Media files must be 12 MB or smaller.", "error");
          ...
          try {
            ...
            fileInput.value = ""; setStatus(files.length + " file" + (files.length === 1 ? "" : "s") + " uploaded.", "success"); await refresh();
